    ...
    <section className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 md:px-8">
        <div className="max-w-2xl mb-16 border-l-4 border-black pl-6">
          <h2 className="text-3xl md:text-4xl font-bold text-black uppercase tracking-tighter mb-2">PROCESS</h2>
          <p className="text-gray-500 font-medium uppercase tracking-widest text-[10px] md:text-xs">From selection to power-on.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border-2 border-black divide-y-2 md:divide-y-0 md:divide-x-2 divide-black">
          {STEPS.map((step, i) => (
            <div key={i} className="bg-white p-8 md:p-10 hover:bg-gray-50 transition-colors group">
              ...
              <h3 className="text-lg md:text-xl font-black text-black uppercase tracking-tight mb-4">{step.title}</h3>
              <p className="text-[13px] md:text-sm text-gray-600 leading-relaxed font-medium">{step.desc}</p>
            </div>
          ))}
        </div>
      </div>
