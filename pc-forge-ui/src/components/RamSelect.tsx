...
import { fetchRAM } from "../api/pcforge";
import type { RAM } from "../types/pcforge";
...
export default function RamSelect({ motherboardId, selectedId, onSelect }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchRAM(motherboardId)
      .then((data) => setRamList(data.ram))
  ...
  if (loading) return <div className="text-center p-6 animate-pulse text-slate-500 text-sm font-black uppercase">Loading RAM...</div>;
  ...
  return (
    <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 hover:border-slate-600 transition-colors">
      <h2 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2">
        <span className="w-5 h-5 bg-blue-600 text-white flex items-center justify-center rounded-sm text-[10px]">3</span>
        Select RAM
      </h2>
      <select
        value={selectedId || ""}
        onChange={(e) => {
            const ram = ramList.find(r => r.product_id === e.target.value);
            if (ram) onSelect(ram);
        }}
      ...
      >
        <option value="" className="bg-slate-900">-- Choose Memory --</option>
        {ramList.map((ram) => (
          <option key={ram.product_id} value={ram.product_id} className="bg-slate-900 text-white text-sm">
            {ram.product_name} ({ram.module_count}x{ram.module_capacity_gb}GB)
          </option>
        ))}
      </select>
    </div>
