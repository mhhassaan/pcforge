      ...
      <div className="max-w-[1400px] mx-auto">
        <header className="mb-12 md:mb-20 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
            <div className="flex-1 min-w-0">
                <h1 className="text-4xl md:text-6xl font-black text-black dark:text-white uppercase tracking-tighter mb-4 italic leading-none break-words">COMMUNITY GALLERY</h1>
                <div className="h-2 w-32 bg-blue-600 mb-6"></div>
                <p className="text-gray-400 dark:text-slate-500 font-bold uppercase tracking-[0.3em] text-[9px] md:text-[10px]">Verified Architectural Assemblies by Local Enthusiasts.</p>
            </div>
            <Link 
                to="/builder"
                className="w-full md:w-auto flex items-center justify-center gap-3 bg-black dark:bg-blue-600 hover:bg-blue-600 dark:hover:bg-blue-500 text-white font-black py-4 md:py-5 px-10 rounded-none transition-all uppercase tracking-widest text-[10px] shadow-[8px_8px_0px_0px_rgba(37,99,235,1)] dark:shadow-[8px_8px_0px_0px_rgba(37,99,235,0.3)]"
            >
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 md:py-40">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-slate-600 italic">Accessing Hardware Archives...</p>
          </div>
        ) : builds.length > 0 ? (
                  ...
                  <div key={build.id} className="group border-2 border-black dark:border-white/10 bg-white dark:bg-slate-900 hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:hover:shadow-[8px_8px_0px_0px_rgba(37,99,235,0.2)] transition-all flex flex-col min-w-0">
                      ...
                      <div className="p-4 md:p-6 space-y-4 flex-1 flex flex-col min-w-0">
                          <div className="min-w-0">
                              <h3 className="text-sm md:text-base font-black text-black dark:text-white uppercase tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors mb-1 truncate">
                                  {build.title}
                              </h3>
                              <div className="flex items-center gap-2 text-[9px] md:text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest truncate">
          ...
          <div className="mt-20 p-20 border-2 border-black dark:border-white/10 border-dashed bg-gray-50 dark:bg-slate-900/30 flex flex-col items-center justify-center text-center">
               <div className="w-16 h-16 bg-white dark:bg-slate-900 border-2 border-black dark:border-blue-600 flex items-center justify-center mb-6">
                  <Camera size={32} className="text-black dark:text-white" />
               </div>
               <h2 className="text-xl font-black text-black dark:text-white uppercase tracking-[0.3em] mb-4">Awaiting Submissions</h2>
               <p className="text-gray-400 dark:text-slate-500 text-xs font-bold uppercase max-w-sm leading-relaxed">
